...
public class TridentTopologyTest extends BaseTopologyTest {
    ...
    public void testTridentTopology() throws Exception {
        ...
        for (int i = 1; i < 4; i++) {
            ResultSet resultSet = session.execute("INSERT INTO weather.station(id, name) VALUES(?, ?)", "station-" + i, "Foo-Station-" + new Random().nextInt());
        }
        ...
        FixedBatchSpout spout = new FixedBatchSpout(new Fields("weather_station_id", "temperature", "event_time"), 3,
                new Values("station-1", "10", UUID.randomUUID()),
                new Values("station-2", "20", UUID.randomUUID()),
                new Values("station-3", "30", UUID.randomUUID())
        );
        spout.setCycle(true);

        TridentTopology topology = new TridentTopology();
        Stream stream = topology.newStream("cassandra-trident-stream", spout);
