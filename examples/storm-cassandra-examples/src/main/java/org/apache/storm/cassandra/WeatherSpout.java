...
public class WeatherSpout extends BaseRichSpout {
    ...
    private boolean printedMaxQueries;
    ...
    public void declareOutputFields(OutputFieldsDeclarer outputFieldsDeclarer) {
        outputFieldsDeclarer.declare(new Fields("weatherStationId", "eventTime", "temperature"));
    }
    ...
    public void nextTuple() {
        if (emit.get() < maxQueries.get()) {
            long currentCount = emit.incrementAndGet();
            spoutOutputCollector.emit(new Values(stationID, currentCount, (long) new Random().nextInt(90)), currentCount);
        }
        if(!printedMaxQueries) {
            System.out.println("############################# reached limit of ma queries: " + maxQueries);
            printedMaxQueries = true;
        }
    }
}